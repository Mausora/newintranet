...
SET time_zone = "+00:00";
DELIMITER $$
CREATE TRIGGER `Notificaciones` BEFORE UPDATE ON `cabecera` FOR EACH ROW BEGIN
	IF(NEW.estado <> OLD.estado) THEN
		IF(NEW.estado = "00 sin solicitar") THEN
			INSERT INTO emails SET 
				...
				Http://factibilidades@tnasolutions.cl" );
		END IF;
		...
		IF(NEW.estado = "07 en producción") THEN
			INSERT INTO emails SET 
				`enviado` = 0,
			 	`fecha`   = NOW(),
			 	`to`		  = "mauricio@tnagroup.cl,m@tnagroup.cl",
